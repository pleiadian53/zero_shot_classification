...
def make_s_curve(**kargs): 
    ...
    output_file = kargs.get('output_file', "s-curve.tif")
...
def locally_linear(X, **kargs): 
    ...
    for i, method in enumerate(methods):
        ...
        # scatter2(X_proj, y)
        fpath = os.path.join(output_dir, f'X-{method}.tif')
        plt.savefig(fpath, dpi=300)
        plt.close()
    return 
...
def isomap(X, **kargs): 
    ...
    output_dir = kargs.get('output_dir', os.path.join(os.getcwd(), 'output'))
    if not os.path.exists(outputdir): os.makedirs(outputdir) # base directory
    output_file = kargs.get('output_file', "X-isomap.tif")
...
def mds(X, **kargs):
    ...
    output_dir = kargs.get('output_dir', os.path.join(os.getcwd(), 'output'))
    if not os.path.exists(outputdir): os.makedirs(outputdir) # base directory
    output_file = kargs.get('output_file', "X-mds.tif")
...
def spectral(X, **kargs):
    ...
    output_dir = kargs.get('output_dir', os.path.join(os.getcwd(), 'output'))
    if not os.path.exists(outputdir): os.makedirs(outputdir) # base directory
    output_file = kargs.get('output_file', "X-spectral.tif")

    # plot_mode = kargs.get('graph_mode', 'seaborn')
    fpath = os.path.join(plot_dir, output_file) 
...
def tsne(X, **kargs): 
    import collections
    # n_neighbors = 10
    n_components = kargs.get('n_components', 2)
    y = kargs.get('y', None)
    ...
    print('(tsne) dim | X_proj: %s' % str(X_proj.shape))
    t1 = time()
    ...
    outputdir = kargs.get('outputdir', os.path.join(os.getcwd(), 'output'))
    if not os.path.exists(outputdir): os.makedirs(outputdir) # base directory
    # [output]
    fpath = os.path.join(outputdir, f'X-tsne-{identifier}.tif' if identifier else 'X-tsne.tif') 
